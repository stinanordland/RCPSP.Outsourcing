# RCPSP.Outsourcing
# Model A including outsourcing for 18 projects (feasible solution found). 

[Model_A.2.py](https://github.com/user-attachments/files/26929860/Model_A.2.py)
import pandas as pd
import pulp
...
for i in I:
    for j in J_bar[i]:
        model += (
            pulp.lpSum(x[(i, j, t)] for t in feasible_t[(i, j)]) +
...
for i in I:
    for j in J[i]:
        ...
        if j in J_bar[i]:
            start_j += pulp.lpSum(t * y[(i, j, t)] for t in feasible_t[(i, j)])
...
for i in I:
    for j in J[i]:
        expr = pulp.lpSum(t * x[(i, j, t)] for t in feasible_t[(i, j)])
        if j in J_bar[i]:
            expr += pulp.lpSum(t * y[(i, j, t)] for t in feasible_t[(i, j)])
        model += F[i] >= expr
